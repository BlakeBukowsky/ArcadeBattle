...
interface PlayerState {
  ...
  lastHitResult: 'perfect' | 'good' | 'miss' | null; lastHitTime: number;
}
interface RhythmState {
  players: Record<string, PlayerState>;
  arrows: Arrow[]; speed: number;
  canvasWidth: number; canvasHeight: number; hitZoneY: number;
  winner: string | null;
...
function drawArrowShape(c: CanvasRenderingContext2D, cx: number, cy: number, size: number, dir: Direction, color: string) {
    ...
    function draw() {
      ...
      pids.forEach((pid, idx) => {
        ...
        if (p.lastHitResult && resultAge < 500) {
          c.globalAlpha = Math.max(0, 1 - resultAge / 500);
          const rColor = p.lastHitResult === 'perfect' ? '#00ff88' : p.lastHitResult === 'good' ? '#ffff00' : '#ff4444';
          const rText = p.lastHitResult.toUpperCase();
          drawLabel(c, rText, ox + HALF / 2, state.hitZoneY + 40, { color: rColor, font: '16px monospace' });
          c.globalAlpha = 1;
        }

        drawLabel(c, isMe ? 'YOU' : 'OPPONENT', ox + HALF / 2, 20, { color: '#ffffff88', font: '14px monospace' });
        ...
        if (!p.alive) {
          ...
          drawLabel(c, 'OUT!', ox + HALF / 2, H / 2, { color: '#ff4444', font: '24px monospace' });
        }

        c.restore();
      });

      drawLabel(c, `Speed: ${state.speed.toFixed(1)}`, W / 2, H - 8, { color: '#ffaa00', font: '12px monospace' });
